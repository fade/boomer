...
(test hkdf-sha256-test-case-1
  ...
  (let ((ikm (hex-to-bytes "0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b"))
    ...
    ;; Test HKDF-Extract
    (let ((prk (boomer::hkdf-extract salt ikm :digest :sha256)))
      (is (bytes-equal prk expected-prk)
          "HKDF-Extract should produce correct PRK"))
    ;; Test HKDF-Expand
    (let* ((prk (boomer::hkdf-extract salt ikm :digest :sha256))
           (okm (boomer::hkdf-expand prk info 42 :digest :sha256)))
...
(test hkdf-sha256-test-case-2
  ...
  (let ((ikm (hex-to-bytes
    ...
    (let* ((prk (boomer::hkdf-extract salt ikm :digest :sha256))
           (okm (boomer::hkdf-expand prk info 82 :digest :sha256)))
      (is (bytes-equal okm expected-okm)
...
(test aes-128-gcm-encrypt-decrypt
  ...
  (let ((key (hex-to-bytes "feffe9928665731c6d6a8f9467308308"))
    ...
    ;; Test encryption
    (let ((result (boomer::aes-gcm-encrypt key nonce plaintext aad)))
    ...
    (let* ((ciphertext-with-tag (concatenate '(vector (unsigned-byte 8))
           ...
           (decrypted (boomer::aes-gcm-decrypt key nonce ciphertext-with-tag aad)))
...
(test aes-gcm-authentication-failure
  ...
  (let ((key (hex-to-bytes "feffe9928665731c6d6a8f9467308308"))
        ...
        (aad (make-array 0 :element-type '(unsigned-byte 8))))
    (let* ((ciphertext (boomer::aes-gcm-encrypt key nonce plaintext aad))
           ;; Tamper with the ciphertext
           (tampered (copy-seq ciphertext)))
      (setf (aref tampered 0) (logxor (aref tampered 0) #xff))
      (signals boomer::tls-mac-error
        (boomer::aes-gcm-decrypt key nonce tampered aad)))))
...
(test chacha20-poly1305-rfc8439-section-2-8-2
  ...
  (let ((key (hex-to-bytes
    ...
    ;; Test encryption
    (let ((result (boomer::chacha20-poly1305-encrypt key nonce plaintext aad)))
      (is (= (length result) (+ (length plaintext) 16))
    ...
    (let* ((ciphertext-with-tag (concatenate '(vector (unsigned-byte 8))
                                              expected-ciphertext expected-tag))
           (decrypted (boomer::chacha20-poly1305-decrypt key nonce ciphertext-with-tag aad)))
      (is (bytes-equal decrypted plaintext)
...
(test chacha20-poly1305-authentication-failure
  ...
  (let ((key (hex-to-bytes
    ...
    (let* ((ciphertext (boomer::chacha20-poly1305-encrypt key nonce plaintext aad))
           (tampered (copy-seq ciphertext)))
      (setf (aref tampered 0) (logxor (aref tampered 0) #xff))
      (signals boomer::tls-mac-error
        (boomer::chacha20-poly1305-decrypt key nonce tampered aad)))))
...
(test tls13-hkdf-expand-label
  ...
  (let* ((secret (hex-to-bytes
    ...
    (let ((result (boomer::hkdf-expand-label secret "derived"
                                               empty-hash
                                               32 :digest :sha256)))
...
(test constant-time-equal-same
  ...
  (let ((a (hex-to-bytes "0102030405060708"))
    ...
    (is (boomer:constant-time-equal a b)
...
(test constant-time-equal-different
  ...
  (let ((a (hex-to-bytes "0102030405060708"))
    ...
    (is (not (boomer:constant-time-equal a b))
...
(test ct-select-mask-ff
  ...
  (let ((a (hex-to-bytes "0102030405060708"))
    ...
    (is (bytes-equal (boomer::ct-select #xff a b) a))))

(test ct-select-mask-00
  "ct-select with #x00 mask returns second vector"
  (let ((a (hex-to-bytes "0102030405060708"))
        (b (hex-to-bytes "f1f2f3f4f5f6f7f8")))
    (is (bytes-equal (boomer::ct-select #x00 a b) b))))
...
(test ml-kem-768-decaps-branchless
  ...
  (multiple-value-bind (ek dk)
      (boomer::ml-kem-768-keygen)
    (multiple-value-bind (ss ct)
        (boomer::ml-kem-768-encaps ek)
      ;; Valid decapsulation
      (let ((ss-dec (boomer::ml-kem-768-decaps dk ct)))
      ...
      (let ((bad-ct (copy-seq ct)))
        ...
        (let ((bad-ss (boomer::ml-kem-768-decaps dk bad-ct)))
          (is (= (length bad-ss) 32)
...
(test ml-kem-768-deterministic
  ...
  (let ((d (make-array 32 :element-type '(unsigned-byte 8) :initial-element #x42))
    ...
    (multiple-value-bind (ek dk)
        (boomer::ml-kem-768-keygen-internal d z)
      (multiple-value-bind (ss ct)
          (boomer::ml-kem-768-encaps-internal ek m)
        (let ((ss-dec (boomer::ml-kem-768-decaps dk ct)))
          (is (bytes-equal ss ss-dec)
...
(test ml-kem-ntt-roundtrip
  ...
  (let ((poly (boomer::make-ml-kem-poly)))
    ;; Initialize with some values
    (dotimes (i 256)
      (setf (aref poly i) (mod i boomer::+ml-kem-q+)))
    (let ((original (copy-seq poly)))
      ;; Apply NTT and inverse NTT
      (boomer::ntt-forward poly)
      (boomer::ntt-inverse poly)
...
(test hybrid-x25519-ml-kem-768-roundtrip
  ...
  (let ((client-kex (boomer::make-hybrid-x25519-ml-kem-768)))
    ;; Get client's public key share
    (let ((client-share (boomer::hybrid-public-key client-kex)))
      ...
      (multiple-value-bind (server-ss server-share)
          (boomer::hybrid-server-encaps client-share)
        ...
        (let ((client-ss (boomer::hybrid-compute-shared-secret client-kex server-share)))
...
(test ml-kem-poly-arithmetic
  ...
  (let ((a (boomer::make-ml-kem-poly))
        (b (boomer::make-ml-kem-poly)))
    ;; Set some values
    (dotimes (i 256)
      (setf (aref a i) (mod (* i 13) boomer::+ml-kem-q+))
      (setf (aref b i) (mod (* i 7) boomer::+ml-kem-q+)))
    ;; Test a + b - b = a
    (let* ((sum (boomer::poly-add a b))
           (diff (boomer::poly-sub sum b)))
      (is (every #'= a diff)
