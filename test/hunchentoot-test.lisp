...
;;; using boomer with self-signed certificates.
...
(defpackage #:boomer-hunchentoot-test
  (:use #:cl)
  (:export #:run-test
           #:start-server
           #:stop-server))

(in-package #:boomer-hunchentoot-test)
...
;;;; Boomer Hunchentoot Acceptor

(defclass boomer-acceptor (hunchentoot:easy-acceptor)
  ...
  (:default-initargs
   :address *test-host*
   :port *test-port*)
  (:documentation "A Hunchentoot acceptor that uses boomer for TLS 1.3."))

(defmethod hunchentoot:acceptor-ssl-p ((acceptor boomer-acceptor))
  "This acceptor uses SSL/TLS."
  t)

(defmethod hunchentoot:initialize-connection-stream ((acceptor boomer-acceptor) stream)
  "Wrap the connection stream with boomer for TLS 1.3."
...
(hunchentoot:define-easy-handler (hello-handler :uri "/") ()
  ...
  "Hello from Hunchentoot over TLS 1.3 with boomer!")

(hunchentoot:define-easy-handler (info-handler :uri "/info") ()
  (setf (hunchentoot:content-type*) "application/json")
  (format nil "{\"tls_version\": \"1.3\", \"server\": \"boomer + hunchentoot\"}"))

;;;; Server Management

(defun start-server (&key (port *test-port*) (cert *server-cert*) (key *server-key*))
  "Start the Hunchentoot server with boomer."
  (when *acceptor*
    (stop-server))
...
(defun test-https-request (&key (port *test-port*))
  "Make a test HTTPS request to the server using boomer."
  ...
  (let ((socket (usocket:socket-connect *test-host* port
    ...
    (unwind-protect
         (let ((tls-stream (boomer:make-tls-client-stream
                            ...
                            :hostname "localhost"
                            :verify boomer:+verify-none+)))  ; Self-signed cert
           (unwind-protect
                (progn
                  ;; Verify TLS 1.3
                  (let ((tls-version (boomer:tls-version tls-stream))
                        (cipher-suite (boomer:tls-cipher-suite tls-stream)))
                    ;; TLS 1.3 = 0x0304 = 772
...
(defun run-test ()
  ...
  (format t "~%")
  (format t "========================================~%")
  (format t "  Hunchentoot + boomer TLS 1.3 Test  ~%")
  (format t "========================================~%")
