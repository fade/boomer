...
(test spent-stream-refuses-every-input-and-output-entry-point
  ...
  (let* ((stream (make-handover-stream (pure-tls::octet-vector 1 2 3 4) 0))
    ...
    (signals pure-tls::tls-stream-spent (read-byte stream))
    (signals pure-tls::tls-stream-spent
      (read-sequence (pure-tls::make-octet-vector 4) stream))
    (signals pure-tls::tls-stream-spent (write-byte 65 stream))
    (signals pure-tls::tls-stream-spent
      (write-sequence (pure-tls::octet-vector 65 66) stream))
    (signals pure-tls::tls-stream-spent (force-output stream))
    (signals pure-tls::tls-stream-spent (finish-output stream))
    ;; Specific enough to act on, rather than something a caller has to
    ;; recognise by reading the message.
    (handler-case (read-byte stream)
      (pure-tls::tls-stream-spent (refusal)
        (is (typep refusal 'pure-tls:tls-error)
            "The refusal reports under the library's own condition hierarchy")
        (is (search "reading a byte" (princ-to-string refusal))
            "and names the entry point it refused")))))
