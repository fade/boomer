...
;;;;
;;;; The fix records, on each ticket, the hostname (ADN) the minting handshake
      ...
      (unwind-protect
           (let (hs1 hs2 hs3)
             ...
             (let ((tk (pure-tls::session-ticket-cache-get host)))
               (is (and tk (equal (pure-tls::session-ticket-verified-adn tk) host))
                   "Cached ticket must record the verified ADN"))
    ...
    (multiple-value-bind (thread server-info)
      ...
      (unwind-protect
           (progn
             ...
             (let ((tk (pure-tls::session-ticket-cache-get host)))
               (is (and tk (null (pure-tls::session-ticket-verified-adn tk)))
...
(test resumption-cross-adn-fails-closed
  "A ticket whose proven ADN differs from the host being connected to must fail
   closed on resumption, even though the server accepts the PSK -- exercising
   the ADN-equality guard on the resumed certificate-less Finished."
  (let ((pure-tls:*use-windows-certificate-store* nil)
        (pure-tls:*use-macos-keychain* nil)
        (port (allocate-test-port))
        (ca (test-cert-path "resumption-ca.pem"))
    ...
    (multiple-value-bind (thread server-info)
      ...
      (unwind-protect
           (progn
             ...
             (%resumption-client port host pure-tls:+verify-required+ ca)
             ;; Rewrite the cached ticket's proven ADN to a different host while
             ;; leaving it keyed under HOST, so the next resumption offers a
             ;; genuine PSK whose provenance is for the wrong identity.
             (let ((tk (pure-tls::session-ticket-cache-get host)))
               (is (and tk (equal (pure-tls::session-ticket-verified-adn tk) host))
                   "Sanity: minted ticket is provenance-stamped for HOST")
               (setf (pure-tls::session-ticket-verified-adn tk) "other-identity.test"))
             ;; 2nd: server resumes the PSK, but its provenance is cross-ADN.
             (signals pure-tls:tls-certificate-error
               (%resumption-client port host pure-tls:+verify-required+ ca)))
        (pure-tls::session-ticket-cache-clear host)
        (ignore-errors (bt:join-thread thread))))))
